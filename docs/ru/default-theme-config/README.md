...
### Отображение ссылок заголовков для всех страниц <Badge text="0.11.0+"/>
...
::: warning ВНИМАНИЕ
Поскольку `lastUpdated` основывается на `git`, то использование возможно только с `git` репозиторием. Кроме того, поскольку метка времени определяется по коммиту в git, она отобразится только после первого коммита для данной страницы, а обновляться только при последующих коммитах этой страницы.
:::
...
::: tip Примечание
Если не указывать кнопку `Обновить`, то новый service worker станет активным только после закрытия всех [клиентов](https://developer.mozilla.org/en-US/docs/Web/API/Clients). Это означает, что пользователи не смогут увидеть новый контент, до тех пор пока не закроют все вкладки вашего сайта. Но кнопка `Обновить` активирует новый service worker незамедлительно.
:::
...
module.exports = {
  themeConfig: {
    serviceWorker: {
      ...
      // Если установлено в true, значения по умолчанию будут такими:
      // updatePopup: {
...
### Миграция ваших стилей в `style.styl` <Badge text="0.12.0+"/>
